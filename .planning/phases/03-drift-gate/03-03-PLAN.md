...
must_haves:
  truths:
    - "ReverseDepGraph built from Vec<FileResult> in O(n) time"
    - "get_importers returns direct importers of a file path"
    - "expand_affected returns all files importing any changed file"
  artifacts:
    - path: "crates/wtcd-core/src/depgraph.rs"
      provides: "ReverseDepGraph with build, get_importers, expand_affected, importer_count"
  key_links:
    - from: "FileResult.imports"
      to: "ReverseDepGraph.importers"
      via: "inversion of DependencyEdge.source"
---

<objective>
Build a reverse dependency graph from parsed FileResult data. Given a set of changed files, find all files that import them (direct dependents). This is used by the drift engine to expand the change impact set.
</objective>

<implementation>

<step number="1" name="create-depgraph-module">
<action>
1. Add a new module to `crates/wtcd-core/src/lib.rs`:
   ```rust
   pub mod depgraph;
   ```

2. Create `crates/wtcd-core/src/depgraph.rs` with the reverse dependency graph:

   ```rust
   use std::collections::{HashMap, HashSet};
   use crate::types::{DependencyEdge, FileResult};
       ...
       pub fn build(file_results: &[FileResult]) -> Self {
           ...
           for result in file_results {
               ...
               for edge in &result.imports {
                   let imported_path = Self::resolve_import_path(
                       importer_path,
                       &edge.source,
                   );
   ...
   mod tests {
       ...
       fn package_imports_preserved() {
           let files = vec![
               make_file_result("src/app.ts", vec![("lodash", vec!["merge"])]),
           ];
           let graph = ReverseDepGraph::build(&files);
...
3. Verify: `cargo test -p wtcd-core -- depgraph` passes
</action>
<commit>feat(03-03): implement ReverseDepGraph with importer expansion</commit>
<verification>
```bash
cargo build -p wtcd-core
cargo test -p wtcd-core -- depgraph
```
</verification>
</step>
...
# Verify graph module exists
ls crates/wtcd-core/src/depgraph.rs
# Verify key functions
grep -q "expand_affected" crates/wtcd-core/src/depgraph.rs
grep -q "importer_count" crates/wtcd-core/src/depgraph.rs
```
</verification>
...
|-------------|----------|
| DRFT-04 | ReverseDepGraph: expand_affected finds direct dependents of changed files |

## Design Notes

- Placed in `wtcd-core` (not a new crate) because the graph is pure data structure from `FileResult` types that already live in wtcd-core
- Import resolution handles relative imports (`./`, `../`) with TS/JS extension inference
- Package imports (no `.` prefix) kept as-is — the drift engine handles them differently
- Runtime HashMap (D-14) — no disk persistence, built fresh on each invocation
- `expand_affected` excludes files already in the changed set to avoid duplicates
